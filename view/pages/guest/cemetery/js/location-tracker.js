...
class LocationTracker {
  constructor(cemeteryManager) {
    ...
    this.hasShownArrival = false; // Prevent multiple arrival modals
  ...
  computeMapBearing() {
    if (this.headingSmoothed === null || this.headingSmoothed === undefined)
      return null;
  ...
  handleDeviceOrientation(event) {
    ...
    if (heading !== null && heading !== undefined) {
      ...
      if (
        typeof this.headingSmoothed !== "number" ||
        Number.isNaN(this.headingSmoothed)
      ) {
        ...
        if (delta < -180) delta += 360;
        this.headingSmoothed =
          (this.headingSmoothed + this.headingSmoothingFactor * delta + 360) %
          360;
      }
  ...
  async startNavigationToGrave(targetLat, targetLng, graveData) {
    try {
      ...
      this.hasShownArrival = false; // Reset arrival flag
      ...
      if (typeof CustomToast !== "undefined") {
        CustomToast.show(
          "success",
          `Navigation started to ${graveData.deceasedName || "Grave"}`
        );
      }
    } catch (error) {
  ...
  updateNavigation() {
    ...
    const distance = this.cemeteryManager.haversineMeters(
      ...
      this.navigationTarget.lat,
      this.navigationTarget.lng
    );
    ...
    if (distance <= 10 && !this.hasShownArrival) {
      this.handleArrival();
    }
  }
  ...
  // Update next turn instruction (simplified - just show target info)
  updateNextTurnInstruction() {
    const nextTurnElement = document.getElementById("nextTurn");
    if (!nextTurnElement || !this.navigationTarget) return;

    const graveName = this.navigationTarget.data?.deceasedName || "Grave";
    nextTurnElement.innerHTML = `
            <small class="text-muted">Navigating to: <strong>${graveName}</strong></small>
        `;
  }
  ...
  // Handle arrival at destination
  async handleArrival() {
    // Prevent showing multiple arrival modals
    if (this.hasShownArrival) return;
    this.hasShownArrival = true;
    ...
    // Show arrival modal with grave information and photo
  ...
  async showArrivalModal(graveData) {
    try {
      const arrivalModal = new bootstrap.Modal(
        document.getElementById("arrivalModal")
      );
      arrivalModal.show();
  ...
  populateArrivalModal(graveData) {
    ...
    if (graveNumberEl) {
      graveNumberEl.textContent = `Grave Number: ${
        record.grave_number || "N/A"
      }`;
    }
    ...
    if (viewDetailsBtn) {
      viewDetailsBtn.onclick = () => {
        ...
        const arrivalModal = bootstrap.Modal.getInstance(
          document.getElementById("arrivalModal")
        );
        if (arrivalModal) {
          arrivalModal.hide();
