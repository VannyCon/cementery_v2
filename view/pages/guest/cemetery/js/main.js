...
function initializeTermsModal() {
    ...
    acceptTermsBtn.addEventListener('click', function() {
        if (acceptTermsCheckbox.checked) {
            ...
            showToast('Terms and Conditions accepted. You may now use the system.', 'success');
        }
    });
}
    ...
    if (advancedSearchToggle) {
        advancedSearchToggle.addEventListener('click', function() {
            if (advancedSearchPanel.style.display === 'none') {
                ...
                advancedSearchToggle.innerHTML = '<i class="fas fa-chevron-up me-2"></i>Hide Advanced Search';
            } else {
                advancedSearchPanel.style.display = 'none';
                advancedSearchToggle.innerHTML = '<i class="fas fa-cog me-2"></i>Advanced Search';
            }
        });
    }
...
function performMainSearch() {
    ...
    searchBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i><span class="search-btn-text">Searching...</span>';
...
function showInitialSearch() {
    ...
    advancedSearchPanel.style.display = 'none';
    document.getElementById('advancedSearchToggle').innerHTML = '<i class="fas fa-cog me-2"></i>Advanced Search';
...
async function loadRecords() {
    try {
        ...
        const response = await axios.get(`../../../../api/records.php?${params.toString()}`, {
            headers: {
        ...
        if (response.data && response.data.success) {
            ...
            if (response.data.data && typeof response.data.data === 'object' && !Array.isArray(response.data.data)) {
                ...
                Object.keys(response.data.data).forEach(graveId => {
                    if (graveId !== 'meta' && Array.isArray(response.data.data[graveId])) {
...
function getFilteredRecords() {
    ...
    return records.filter(r => {
        ...
        return deceasedName.includes(q) || 
...
function displayDesktopTable(list) {
    ...
    list.forEach(record => {
        ...
        row.innerHTML = `
            ...
            <td class="text-nowrap">${window.Utils.formatDate(record.date_of_birth)}</td>
            <td class="text-nowrap">${window.Utils.formatDate(record.date_of_death)}</td>
            <td class="text-nowrap">${window.Utils.formatDate(record.burial_date)}</td>
            <td class="text-nowrap">${record.next_of_kin || ''}</td>
            <td class="text-nowrap">${record.contact_info || ''}</td>
            <td class="text-truncate" style="max-width: 200px;" title="${record.notes || ''}">${record.notes || ''}</td>
    ...
    card.innerHTML = `
        ...
        <div class="info-grid">
            <div class="info-item">
                ...
                <div class="info-value">${window.Utils.formatDate(record.date_of_birth) || 'N/A'}</div>
            </div>
            <div class="info-item">
                <div class="info-label">Date of Death</div>
                <div class="info-value">${window.Utils.formatDate(record.date_of_death) || 'N/A'}</div>
            </div>
            <div class="info-item">
                <div class="info-label">Burial Date</div>
                <div class="info-value">${window.Utils.formatDate(record.burial_date) || 'N/A'}</div>
            </div>
            <div class="info-item">
        ...
        <div class="card-actions">
            <button class="btn btn-outline-primary" onclick="viewRecord(${record.id})" title="View Details">
                <i class="fas fa-eye me-1"></i>View
            </button>
            <button class="btn btn-outline-info" onclick="locateRecord(${record.id})" title="Locate on Map">
...
function locateRecordOnMap(record) {
    ...
    if (!lat || !lng) {
        CustomToast && CustomToast.warning('Warning', 'Location coordinates not available for this grave');
        return;
    }
...
function viewRecord(id) {
    ...
    if (window.offlineDataCache) {
        ...
        console.log('Record cached for offline access:', record.grave_number || record.deceased_name);
    }
...
function displayImageGallery(record) {
    // Build image gallery from image_path
    const imagesCsv = record.image_path ? String(record.image_path) : '';
