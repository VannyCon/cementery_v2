...
class CemeteryManager {
  ...
  setupMapEventHandlers() {
    ...
    this.map.on("click", (e) => {
      ...
      if (!this.graphNodes.length) {
        this.setRouteInfo("No routing graph available.");
        return;
      }
      ...
      startLocationBtn.addEventListener("click", async () => {
        try {
          if (
            ...
            !this.locationTracker.isLocationTrackingActive()
          ) {
            await this.locationTracker.startLocationTracking();
            startLocationBtn.innerHTML =
            ...
            startLocationBtn.innerHTML =
              '<i class="fas fa-location-arrow"></i><span class="d-none d-sm-inline">My Location</span>';
        ...
        if (gravePlotData) {
          try {
            ...
            if (this.locationTracker && data.lat && data.lng) {
              this.locationTracker.startNavigationToGrave(
                data.lat,
                data.lng,
                data
              );
          ...
          if (typeof CustomToast !== "undefined") {
            CustomToast.show(
              "warning",
              "No grave plot selected. Please search for a grave first."
            );
          }
        }
      });
  ...
  async loadData() {
    ...
    try {
      const response = await axios.get(
        `../../../../api/cemetery.php?action=getGuestMapData`,
        {
          headers: {
  ...
  clearLayers() {
    ...
    const sourcesToRemove = ["cemeteries", "graves", "grave-plots", "routes"];
  ...
  renderRoute(coords, distanceMeters) {
    ...
    // Convert coords from [lat, lng] to [lng, lat] for MapLibre GL
  ...
  async useMyLocation() {
    try {
      ...
      userLocationEl.innerHTML =
        '<div class="user-location-dot"><div class="user-location-pulse"></div></div>';
      ...
      // If routing graph exists, automatically set as start point
      if (this.graphNodes.length > 0) {
        const snapped = this.findNearestGraphNode({ lat, lng });
        if (snapped) {
          this.setStartPoint(snapped);
          this.setRouteInfo(
            `Your location set as start point. Click anywhere to set destination and get route.`
          );
        } else {
          this.setRouteInfo(
            `Location found but no nearby routing points available.`
          );
        }
      } else {
        this.setRouteInfo(
          `Location found with ${Math.round(accuracy)}m accuracy.`
        );
      }
      ...
      if (typeof CustomToast !== "undefined") {
        CustomToast.show(
          "success",
          `Your location is now the start point! Click destination to get route.`
        );
      }
  ...
  setLocationAsStart(lat, lng) {
    if (this.graphNodes.length === 0) {
      if (typeof CustomToast !== "undefined") {
        CustomToast.show("warning", "No routing graph available.");
      }
      return;
    }
  ...
  getBurialDetails() {
    try {
      ...
      const { lat, lng, graveNumber, deceasedName, record, boundary_coords } =
        gravePlotData;
      ...
      if (lat && lng && !isNaN(lat) && !isNaN(lng)) {
        ...
        if (typeof showToast !== "undefined") {
          showToast(
            `Located grave ${graveNumber} for ${deceasedName}`,
            "success"
          );
        }
      ...
      console.error(
        "Error reading grave plot location from sessionStorage:",
        error
      );
      // Clear invalid data
      sessionStorage.removeItem("gravePlotLocation");
  ...
  showGraveLocationPopup(lat, lng, graveNumber, deceasedName, record = null) {
    ...
    if (record) {
      if (record.date_of_birth) {
        additionalInfo += `<p class="mb-1"><strong>Date of Birth:</strong> ${window.Utils.formatDate(
          record.date_of_birth
        )}</p>`;
      }
      if (record.date_of_death) {
        additionalInfo += `<p class="mb-1"><strong>Date of Death:</strong> ${window.Utils.formatDate(
          record.date_of_death
        )}</p>`;
      }
      if (record.burial_date) {
        additionalInfo += `<p class="mb-1"><strong>Burial Date:</strong> ${window.Utils.formatDate(
          record.burial_date
        )}</p>`;
      }
    ...
    const popupContent = `
            <div class="grave-location-popup" style="min-width: 250px;">
                ...
                <p class="mb-1"><strong>Grave Number:</strong> ${
                  graveNumber || "Unknown"
                }</p>
                <p class="mb-1"><strong>Deceased:</strong> ${
  ...
  adjustPitchForOrientation() {
    ...
    if (Math.abs(currentPitch - newPitch) > 5) {
      ...
      console.log(
        `Orientation changed - adjusting pitch from ${currentPitch}° to ${newPitch}°`
      );
    }
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const scripts = ["https://unpkg.com/maplibre-gl@3.6.2/dist/maplibre-gl.js"];

  const styles = ["https://unpkg.com/maplibre-gl@3.6.2/dist/maplibre-gl.css"];
