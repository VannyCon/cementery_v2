...
class LayerManager {
    ...
    // Show annotation popup with details and actions
    showAnnotationPopup(feature, lngLat) {
        const annotation = feature.properties;
        
        const popupContent = `
            <div>
                <p><strong>${this.escapeHtml(annotation.label)}</strong><br>
                ${annotation.notes ? `${this.escapeHtml(annotation.notes)}` : ''}
                </p>
                
                <div class="mt-1">
                    <button class="btn btn-sm btn-primary" onclick="cemeteryManager.layerManager.editAnnotation(${annotation.id})">
                        <i class="fas fa-edit"></i> Edit
                    </button>
                    <button class="btn btn-sm btn-danger" onclick="cemeteryManager.layerManager.deleteAnnotation(${annotation.id})">
    ...
    handleAnnotationCreate(feature) {
        ...
        // Show annotation modal for details
        if (this.cemeteryManager.modalManager) {
            this.cemeteryManager.modalManager.showAnnotationModal(feature);
        } else {
    ...
    addAnnotationEventListeners() {
        ...
        this.map.on('click', 'annotations-fill', (e) => {
            // Only show popup if we're in the annotations tab
