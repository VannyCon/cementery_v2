...
class RoadManager {
    ...
    // Show road popup with details and actions
    showRoadPopup(feature, lngLat) {
        const road = feature.properties;
        
        const popupContent = `
            <div>
                <strong>${this.escapeHtml(road.road_name)}</strong><br>
                <div class="mt-2">
                    <button class="btn btn-sm btn-primary" onclick="cemeteryManager.roadManager.editRoad(${road.id})">
                        <i class="fas fa-edit"></i> Edit
                    </button>
                    <button class="btn btn-sm btn-danger" onclick="cemeteryManager.roadManager.deleteRoad(${road.id})">
    ...
    addRoadEventListeners() {
        ...
        this.map.on('click', 'roads', (e) => {
            // Only show popup if we're in the roads tab
            if (window.tabs === 'roads') {
                const feature = e.features[0];
                this.showRoadPopup(feature, e.lngLat);
            }
        });
    ...
    clearRoadLayers() {
        ...
        // Remove all road layers (must remove layers before source)
        const roadLayers = ['roads-top', 'roads-border', 'roads'];
