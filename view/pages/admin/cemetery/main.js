...
class CemeteryManager {
    ...
    initializeMap() {
        ...
        this.map.on('load', () => {
            ...
            // Add 3D grave model layer
            // this.addGraveModelLayer();
    ...
    // Add a Three.js custom layer to render the local grave.gltf on the map
    addGraveModelLayer() {
        // Ensure THREE is available (loaded via script tag below)
        if (typeof THREE === 'undefined') {
            console.warn('THREE not yet loaded; retrying grave model layer init...');
            setTimeout(() => this.addGraveModelLayer(), 200);
            return;
        }

        // Avoid double-adding
        if (this.map.getLayer('grave-3d-model')) return;

        // Position the model at current map center (can be adjusted as needed)
        const modelOrigin = [this.map.getCenter().lng, this.map.getCenter().lat];
        const modelAltitude = 0;
        const modelRotate = [Math.PI / 2, 0, 0];

        const merc = maplibregl.MercatorCoordinate.fromLngLat(modelOrigin, modelAltitude);
        ...
        const customLayer = {
            ...
            onAdd: (map, gl) => {
                ...
                this._threeScene.add(light1);
                const light2 = new THREE.DirectionalLight(0xffffff);
                light2.position.set(0, 70, 100).normalize();
                this._threeScene.add(light2);

                // GLTF Loader (examples/js variant exposes THREE.GLTFLoader)
                const loader = new THREE.GLTFLoader();
                loader.load('rect_grave.glb', (gltf) => {
                    this._threeScene.add(gltf.scene);
                }, undefined, (err) => {
                    console.error('Failed to load grave.gltf:', err);
                });

                // Use MapLibre canvas GL context
                this._threeRenderer = new THREE.WebGLRenderer({ canvas: map.getCanvas(), context: gl, antialias: true });
                this._threeRenderer.autoClear = false;
            },
            render: (gl, matrix) => {
                // matrix is a Float32Array (MapLibre v3) representing camera projection * view
                ...
                this._threeCamera.projectionMatrix = m.multiply(l);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof maplibregl !== 'undefined' && typeof THREE !== 'undefined' && typeof THREE.GLTFLoader !== 'undefined') {
