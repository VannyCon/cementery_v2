...
class LocationTracker {
  ...
  handleLocationUpdate(position) {
    ...
    if (this.followMode && courseHeading !== null && speed > 1.5) {
      ...
      if (
        ...
        this.headingSmoothed =
          (this.headingSmoothed + movementSmoothing * delta + 360) % 360;
      }
    }
  ...
  async calculateRouteToTarget() {
    ...
    try {
      ...
      if (typeof CustomToast !== "undefined") {
        CustomToast.show(
          "warning",
          "Could not calculate route. Showing direct path."
        );
      }
  ...
  updateNextTurnInstruction() {
    ...
    if (currentInstruction && minDistance < 50) {
      // Within 50 meters of next instruction
      nextTurnElement.innerHTML = `
                <small class="text-muted">${
                  currentInstruction.instruction
                } in ${Math.round(minDistance)}m</small>
            `;
    } else {
      nextTurnElement.innerHTML =
        '<small class="text-muted">Continue on current path</small>';
    }
  }
